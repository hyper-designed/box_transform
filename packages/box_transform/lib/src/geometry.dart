...
abstract final class Geometry {
  ...
  static double _lerp(double a, double b, double t) => a * (1.0 - t) + b * t;
}
  ...
  static Dimension? lerp(Dimension? a, Dimension? b, double t) {
    if (b == null) {
      ...
      if (a == null) {
        ...
        return Dimension(Geometry._lerp(a.width, b.width, t),
            Geometry._lerp(a.height, b.height, t));
      }
    }
  }
...
class Box {
  ...
  ///
  /// [rotation] is in radians, clockwise, around the box center. Defaults
  /// to 0 (axis-aligned).
  const Box.fromLTRB(
    this.left,
    this.top,
  ...
  /// height.
  ///
  /// To construct a [Box] from a [Vector2] and a [Dimension], you can use the
  ...
  const Box.fromLTWH(
    ...
    double height, {
    double rotation = 0.0,
  }) : this.fromLTRB(left, top, left + width, top + height, rotation: rotation);
  ...
  Box.fromPoints(Vector2 a, Vector2 b, {double rotation = 0.0})
      : this.fromLTRB(
          math.min(a.x, b.x),
          math.min(a.y, b.y),
          math.max(a.x, b.x),
  ...
  /// Rotation in radians, clockwise, around the box center. Zero = axis-aligned.
  ...
  Box shift(Vector2 vec) => Box.fromLTRB(
        left + vec.x,
        top + vec.y,
        right + vec.x,
        bottom + vec.y,
  ...
  Box inflate(double delta) => Box.fromLTRB(
        left - delta,
        top - delta,
        right + delta,
        bottom + delta,
  ...
  static Box? lerp(Box? a, Box? b, double t) {
    if (b == null) {
      ...
      if (a == null) {
        ...
        return Box.fromLTRB(
          Geometry._lerp(a.left, b.left, t),
          Geometry._lerp(a.top, b.top, t),
          Geometry._lerp(a.right, b.right, t),
          Geometry._lerp(a.bottom, b.bottom, t),
        );
      }
    }
  }
  ...
  @override
  int get hashCode => Object.hash(left, top, right, bottom, rotation);

  @override
  String toString() => rotation == 0.0
      ? 'Box.fromLTRB(${left.toStringAsFixed(1)}, ${top.toStringAsFixed(1)}, ${right.toStringAsFixed(1)}, ${bottom.toStringAsFixed(1)})'
      : 'Box.fromLTRB(${left.toStringAsFixed(1)}, ${top.toStringAsFixed(1)}, ${right.toStringAsFixed(1)}, ${bottom.toStringAsFixed(1)}, rotation: ${rotation.toStringAsFixed(4)})';
}
